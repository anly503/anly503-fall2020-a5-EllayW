...
It is also interactive between each other. By clicking one bar, we can elaborate any parts with details. For example, choosing bar with different color will result in the display of different households. Since we use `consumption` and `house` in both plots, make them interactive will be the best choice due to convenience. 

By this plot, we can explore the different consumption details by weekdays/time periods daily and appliances. Since bar plots will show the difference clearly, those are chosen as below. Pie chart/line plot could make it better but unfortunately, pie charts are not supported in Altair in python.
